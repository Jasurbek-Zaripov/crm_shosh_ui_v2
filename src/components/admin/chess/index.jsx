...
function ChessComponent() {
  ...
  const [openInstantModal, setInstantModal] = useState(false);
  ...
  const OrderGetFilterNot = OrderGet.filter(
    (e) => e.status_client == "not_active"
  );
  const OrdersGetFilterData = OrderGetFilterNot.filter(
    (e) => e.arrival_date.slice(0, 10) == DateFormat(TodayDate)
  );
  const OrdersGetFilterData2 = OrderGetFilter.filter(
  ...
  const RoomsFilter = RoomsGets.filter(
    (elem) => elem.filial?.filial_name == dataUser.filial.filial_name
  );
      ...
      <div className={styles.top}>
        ...
        >
          ...
          <Button onClick={() => setInstantModal(true)}>
            <span className={styles.fleshIcon}>{t("сhess.23")}</span>
          </Button>
        </div>
      </div>
      {/* <TableChess/> */}
