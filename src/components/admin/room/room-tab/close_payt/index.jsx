...
const PaytClose = ({ open, HandleClose, HandleSubmits }) => {
  ...
  const [value, setValue] = useState(
    dayjs(dataFind.map((elem) => elem.arrival_date)[0])
  );
  ...
  const date1 = new Date(
    DateFormat(dataFind.map((elem) => elem.arrival_date)[0])
  ).getTime();
  ...
  const HandleSubmit = async () => {
    const body = {
      total_payable:
        Number(dataFind.map((elem) => elem.total_payable)) -
        (Number(dataFind.map((elem) => elem.booking)) -
          Number(dataFind.map((elem) => elem.definition)) *
            Number(NumberRights)),
      paid:
        Number(dataFind.map((elem) => elem.paid)) -
        (Number(dataFind.map((elem) => elem.booking)) -
          Number(dataFind.map((elem) => elem.definition)) *
            Number(NumberRights)),
      departure_date: DateFormat2(value2.$d),
    };
    await axios.put(`${API_URL}/orders/${id}`, body).then((res) => res);
    await dispatch(
      ChangePost({
    ...
    await axios.put(`${API_URL}/orders/${id}`, bodyrooms2).then((res) => res);
    await axios
      .put(
        `${API_URL}/rooms/${dataFind.map((elem) => elem.rooms?.id)}`,
        bodyrooms
      )
      .then((res) => res);
    ...
    await axios.put(`${API_URL}/orders/${id}`, body2).then((res) => res);
    ...
    >
      <div className={styles.Wrapper}>
        <div className={styles.Calendar}>
          <span>
            {t("appilcation.table.2")} :{" "}
            {dataFind.map((elem) => elem.arrival_date.slice(0, 10))}
          </span>
          ...
          {dataFind.map((elem) =>
            elem.debt != 0 ? (
              <div className={styles.Wrapper}>
                ...
                <h3>
                  {t("Room.30")} :{" "}
                  {Number(dataFind.map((elem) => elem.debt)) - Number(price2)}{" "}
                  {t("Room.34")}
                </h3>
              </div>
            ) : null
