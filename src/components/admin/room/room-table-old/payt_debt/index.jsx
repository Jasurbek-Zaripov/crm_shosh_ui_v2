...
const Refund = ({open , HandleClose}) => {
  ...
  const data = JSON.parse(window.localStorage.getItem("AuthDataUser"));
    ...
    const HandleSubmit = async () =>{
      ...
      await  axios.put(`https://api.hotelshoshmodern.uz/oldorders/${dataFind.map(elem => elem.old.map(e => e.id)[0])[0]}` , body3)
      .then(res => res)
      if (dataFind.map(elem => elem.debt) > 0) {
        await  axios.put(`https://api.hotelshoshmodern.uz/orders/${id}` , body)
        .then(res => res)
      }else {
        await  axios.put(`https://api.hotelshoshmodern.uz/orders/${id}` , body2)
        .then(res => res)
      }
      await dispatch(
        ChangePost({
