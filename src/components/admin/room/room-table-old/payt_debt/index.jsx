...
const Refund = ({ open, HandleClose }) => {
  ...
  const HandleSubmit = async () => {
    ...
    const body2 = {
      total_payable:
        Number(dataFind.map((elem) => elem.total_payable)) - Number(price),
      paid: Number(dataFind.map((elem) => elem.paid)) - Number(price),
    };
    const body3 = {
      refund:
        Number(dataFind.map((elem) => elem.old.map((e) => e.refund)[0])[0]) -
        Number(price),
      total_payable:
        Number(
          dataFind.map((elem) => elem.old.map((e) => e.total_payable)[0])[0]
        ) - Number(price),
      booking:
        Number(dataFind.map((elem) => elem.old.map((e) => e.booking)[0])[0]) -
        Number(price),
      paid:
        Number(dataFind.map((elem) => elem.old.map((e) => e.paid)[0])[0]) -
        Number(price),
    };
    await axios
      .put(
        `${API_URL}/oldorders/${
          dataFind.map((elem) => elem.old.map((e) => e.id)[0])[0]
        }`,
        body3
      )
      .then((res) => res);
  ...
  return (
    ...
    >
      <div className={styles.Wrapper}>
        ...
        <h3>
          {t("Finance.5")} :{" "}
          {Number(dataFind.map((elem) => elem.old.map((e) => e.refund)[0])) -
            Number(price)}{" "}
          {t("Room.34")}
        </h3>
