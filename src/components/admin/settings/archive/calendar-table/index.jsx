  ...
  { 
    Orders.map((elem) => 
      ...
      data.push({
        id: elem.id,
        key: elem.key,
        number: <NavLink className={styles.tablink} to={`/admin/room/${elem.id}`}>{elem.rooms?.rooms}</NavLink>,
        name: (
          <>
        <NavLink style={{marginLeft:"10px"}} className={styles.tablink} to={`/admin/room/${elem.id}`}>{elem.users[0]?.name}</NavLink> 
        
        <NavLink style={{marginLeft:"10px"}} className={styles.tablink} to={`/admin/room/${elem.id}`}>{elem.users[0]?.surname}</NavLink>

        <NavLink style={{marginLeft:"10px"}} className={styles.tablink} to={`/admin/room/${elem.id}`}>{elem.users[0]?.father_name}</NavLink>
        </>
        ),
        country: (
        ...
        dateofarrival: (
          <NavLink className={styles.tablink} to={`/admin/room/${elem.id}`}>{elem.arrival_date.slice(0 , 10)}</NavLink>
        ),
        departuredate: (
          <NavLink className={styles.tablink} to={`/admin/room/${elem.id}`}>{elem.departure_date.slice(0 , 10)}</NavLink>
        ),
        numberofpeople: (
          <NavLink className={styles.tablink} to={`/admin/room/${elem.id}`}>{elem.count_users}</NavLink>
        ),
        tariff: <NavLink className={styles.tablink} to={`/admin/room/${elem.id}`}>{Number(elem.definition).toLocaleString()}</NavLink>,
        accrued: (
          <NavLink className={styles.tablink} to={`/admin/room/${elem.id}`}>{Number(elem.total_payable).toLocaleString()}</NavLink>
        ),
        paid: (
          elem.paid ? <NavLink className={styles.tablinkActiveColor} to={`/admin/room/${elem.id}`}>{Number(elem.paid).toLocaleString()}</NavLink>:<NavLink className={styles.tablink} to={`/admin/room/${elem.id}`}>{Number(elem.paid).toLocaleString()}</NavLink>
        ),
        type:(
          <NavLink className={styles.tablink} to={`/admin/room/${elem.id}`}>{elem.rooms.type}</NavLink>
        ),
        passport:(
          <NavLink className={styles.tablink} to={`/admin/room/${elem.id}`}>{elem.users.map(elem => elem.seriya)[0]} <span>{elem.users.map(elem => elem.number)[0]}</span></NavLink>
        ),
        empytbuys:(
          <NavLink className={elem.status == "empty" ? styles.active  : styles.noactive} to={`/admin/room/${elem.id}`} >{elem.status == "empty" ? `${t("Finance.table.16")}` : `${t("Finance.table.17")}` } </NavLink>
        ),
        debtaccrual: (
          elem.debt ?
          <NavLink className={styles.tablinkActive} to={`/admin/room/${elem.id}`}>{Number(elem.debt).toLocaleString()}</NavLink>:<NavLink className={styles.tablink} to={`/admin/room/${elem.id}`}>{Number(elem.debt).toLocaleString()}</NavLink>
        ),
        company: (
