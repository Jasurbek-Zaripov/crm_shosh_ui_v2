...
export default function FormToInstantBooking() {
  ...
  const onChangeDataPicker = (_, formattedDate) => {
    form.setFieldsValue({
      days: Math.ceil(
        (new Date(formattedDate[1]).getTime() -
          new Date(formattedDate[0]).getTime()) /
          dayInMs
      ),
    });
  };
  ...
  const onFinish = (formData) => {
    ...
    OrderCreate({
      ...
      departure_date: dayjs(formData.date[1]).format(dateFormat),
      count_users: "",
      company: "",
      total_payable: "",
      booking: "",
      ...
      RoomUpdate(+formData.room, { status: "busy" }).catch(() =>
        messageError()
      );
      UserCreate({
        name: formData.name,
    ...
    <div>
      ...
      >
        <Row gutter={16}>
          ...
          <Col span={8}>
            <Form.Item name="room" label={t("application_add.0")}>
              <Select
                showSearch
          ...
          <Col span={8}>
            <Form.Item name="days" label={t("application_add.1")}>
              <Input />
            </Form.Item>
          </Col>

          <Col span={8}>
            <Form.Item name="date" label={t("application_add.9")}>
              <RangePicker
                showTime={{
                  format: "HH:mm",
                }}
          ...
          <Col span={8}>
            <Form.Item name="address" label={t("application_add.31")}>
              <Input />
            </Form.Item>
          </Col>
        </Row>
